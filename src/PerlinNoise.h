...
// https://github.com/DrYaling/PCGForUnity/blob/8aa53771cd0ae6e2b6f3cfb1da5724b8297f6d11/cppLib/code/Generator/Noises/PerlinNoise.h
...
namespace PerlinNoise {
	...
	static inline float noise(plfloat x, plfloat y, plfloat z)
	{
		...
		ix = (int)x; x -= (plfloat)ix;
		iy = (int)y; y -= (plfloat)iy;
		iz = (int)z; z -= (plfloat)iz;
		...
		aa0 = (float)perm[aa]; aa1 = (float)perm[aa + 1];
		ab0 = (float)perm[ab]; ab1 = (float)perm[ab + 1];
		ba0 = (float)perm[ba]; ba1 = (float)perm[ba + 1];
		bb0 = (float)perm[bb]; bb1 = (float)perm[bb + 1];
...
#define Abs(x) ((x>0)?x:-x)
	static inline float noise(plfloat x, plfloat y)
	{
		x = Abs(x);
		y = Abs(y);
		...
		x -= (plfloat)floorX;                                // FIND RELATIVE X,Y,Z
		y -= (plfloat)floorY;                                // OF POINT IN CUBE.
